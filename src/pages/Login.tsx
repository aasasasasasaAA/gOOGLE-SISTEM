  ...
  const loadAuthUrl = async () => {
    try {
      ...
      if (error.response?.status === 503) {
        setAuthUrl(''); // Keep button disabled
        setConfigError('O sistema ainda não foi configurado com as credenciais do Google Ads API. Entre em contato com o administrador.');
      } else {
        setConfigError('Erro ao conectar com o servidor. Verifique sua conexão.');
      }
    }
  };
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="bg-white py-8 px-6 shadow-lg rounded-xl">
            ...
            <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg flex items-start space-x-3">
              ...
              <div>
                <h3 className="text-sm font-medium text-red-800">
                  {configError ? 'Configuração Necessária' : 'Erro de Autenticação'}
                </h3>
                <p className="text-sm text-red-600 mt-1">{configError || error}</p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-6">
            ...
            {/* Setup Instructions or Login Button */}
            {configError ? (
              <div className="space-y-4">
                <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <h4 className="text-sm font-medium text-yellow-800 mb-2">Passos para Configuração:</h4>
